...
namespace Src.Camera
{
    public class CameraMovement : MonoBehaviour
    {
        ...
        {
            _touchStart = _cam.ScreenToWorldPoint(finger.currentTouch.startScreenPosition);
        }
        ...
        private void OnDrag(Finger finger)
        {
            ...
            Vector3 difference =
                _cam.ScreenToWorldPoint(finger.currentTouch.screenPosition)
                - _cam.transform.position;
            
            Debug.Log(_cam.ScreenToWorldPoint(finger.screenPosition));
            
            Debug.Log(difference);
            Debug.Log("-----------");

            _cam.transform.position = _touchStart - difference;
